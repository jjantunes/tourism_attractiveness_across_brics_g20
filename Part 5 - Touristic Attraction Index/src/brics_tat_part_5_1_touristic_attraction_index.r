...
## Data source
vec_src  = c("Henley Passaport Index",
...
write_xlsx(db_desc, file.path(path_output, "table_descriptive_stats_copras.xlsx"))
...
## Criteria weights
vec_wei <- do.call(c, map(mat_dec, \(x) entropy(discretize(x, 10))))
vec_wei <- (1 - vec_wei) / sum(1 - vec_wei)

## MCDM result
mcdm_res <- COPRAS(x_mat = as.matrix(mat_dec), w = vec_wei, crit = vec_dir)

## Create data frame
db_res <- db_pairs %>% select(origin, destination) %>% mutate(score = mcdm_res/100)

## Save result
write_xlsx(db_res, file.path(path_output, "table_touristic_attraction_index.xlsx"))

## Save weights
vec_wei %>%
  data.frame(variables = names(.), weights = .) %>% 
  data.frame(row.names = NULL) %>%
  write_xlsx(file.path(path_output, "table_information_entropy_weights.xlsx"))
